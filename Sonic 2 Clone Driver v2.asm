...
;.driverinput:
	...
	clr.b	SMPS_RAM.v_variables.f_voice_selector(a6)
	tst.b	SMPS_RAM.v_variables.f_stopmusic(a6)			; Is music paused?
	bne.w	DoPauseMusic			; If yes, branch
	move.b	SMPS_RAM.v_variables.v_fadeout_counter(a6),d0
	beq.s	.skipfadeout
	bsr.w	DoFadeOut
; loc_71BA8:
.skipfadeout:
	btst	#f_fadein_flag,SMPS_RAM.v_variables.misc_flags(a6)
	beq.s	.skipfadein
	bsr.w	DoFadeIn
; loc_71BB2:
.skipfadein:
	tst.l	SMPS_RAM.v_variables.v_playsnd1(a6)			; Is a music or sound queued for played?
	beq.s	.nosndinput			; If not, branch
	bsr.w	Sound_Play
; loc_71BBC:
.nosndinput:
	tst.b	SMPS_RAM.v_variables.v_playsnd0(a6)			; Is song queue set for silence?
	beq.s	.nonewsound			; If yes, branch
	bsr.w	Sound_ChkValue
; loc_71BC8:
.nonewsound:
    if SMPS_EnableSpinDashSFX
	tst.b   SMPS_RAM.v_variables.v_spindash_timer(a6)
	beq.s	.notimer
	subq.b	#1,SMPS_RAM.v_variables.v_spindash_timer(a6)
.notimer:
    endif
	...
	btst	#f_force_pal_tempo,SMPS_RAM.v_variables.misc_flags(a6)	; is this song forced to play slow on PAL consoles?
	bne.s	.updatemusictracks			; if so, skip the double-update
	subq.b	#1,SMPS_RAM.v_variables.v_pal_audio_countdown(a6)		; subtract 1 from the PAL countdown
	bne.s	.updatemusictracks			; if the PAL countdown is not 0, we are not ready to double-update, branch
	move.b	#5,SMPS_RAM.v_variables.v_pal_audio_countdown(a6)		; if the countdown is now at 0, reset the counter...
	bset	#f_doubleupdate,SMPS_RAM.v_variables.misc_flags(a6)		; ...and then set the double-update flag
...
; loc_71BD4:
.dacdone:
	bclr	#f_updating_dac,SMPS_RAM.v_variables.misc_flags(a6)
...
.bgmpsgnext:
	...
	bclr	#f_doubleupdate,SMPS_RAM.v_variables.misc_flags(a6)	; Clear double-update flag
	bne.s	.updatemusictracks		; Was the flag set? If so, double-update

;.updatesfxtracks:
	move.b	#$80,SMPS_RAM.v_variables.f_voice_selector(a6)					; Now at SFX tracks
...
.sfxpsgnext:
	...
	move.b	#$40,SMPS_RAM.v_variables.f_voice_selector(a6)	; Now at special SFX tracks
	lea	SMPS_Track.len(a5),a5
	tst.b	SMPS_Track.PlaybackControl(a5)	; Is track playing?
...
UpdateDAC:
	...
	bset	#f_updating_dac,SMPS_RAM.v_variables.misc_flags(a6)	; Set flag to indicate this is the DAC
...
DoPauseMusic:
	...
	cmpi.b	#2,SMPS_RAM.v_variables.f_stopmusic(a6)
	beq.w	locret_71EFE
	move.b	#2,SMPS_RAM.v_variables.f_stopmusic(a6)
	bsr.w	FMSilenceAll
	bsr.w	PSGSilenceAll
	; From Vladikcomper:
...
DoUnpauseMusic:
	clr.b	SMPS_RAM.v_variables.f_stopmusic(a6)
    ...
    if SMPS_EnableSpecSFX
	move.b	#$40,SMPS_RAM.v_variables.f_voice_selector(a6)			; Now at SFX tracks
	lea	SMPS_RAM.v_spcsfx_fm_tracks(a6),a5
	moveq	#SMPS_SPECIAL_SFX_FM_TRACK_COUNT-1,d7	; 1 FM
	bsr.s	ResumeTrack
    endif

	clr.b	SMPS_RAM.v_variables.f_voice_selector(a6)			; Now at SFX tracks
	; From Vladikcomper:
...
Sound_Play:
	tst.b	SMPS_RAM.v_variables.v_playsnd0(a6)		; Is v_variables.v_playsnd0 a $00 (empty)?
	bne.s	locret_71F4A		; If yes, branch
	lea	SMPS_RAM.v_variables.v_playsnd1(a6),a1	; Load music track number
	move.b	SMPS_RAM.v_variables.v_sndprio(a6),d3	; Get priority of currently playing SFX
...
.lowerpriority:
	...
	bmi.s	locret_71F4A
	move.b	d3,SMPS_RAM.v_variables.v_sndprio(a6)	; Set new sound priority
	rts
...
.queueinput:
	move.b	d1,SMPS_RAM.v_variables.v_playsnd0(a6)	; Queue sound for play
locret_71F4A:
	rts
...
;    endif
	...
	tst.b	SMPS_RAM.v_variables.f_1up_playing(a6)	; Is a 1-up music playing?
...
;	clr.b	SMPS_RAM.v_variables.v_sndprio(a6)		; Clear priority (S2 removes this one)
...
.bgmnot1up:
	moveq	#0,d0
	move.b	d0,SMPS_RAM.v_variables.f_1up_playing(a6)
	move.b	d0,SMPS_RAM.v_variables.v_fadein_counter(a6)
	move.b	d0,SMPS_RAM.v_variables.v_fadeout_counter(a6)	; Clownacy | (From S2)
; loc_7202C:
.bgm_loadMusic:
	...
	move.l	(a4,d7.w),d1
	move.b	(a4,d7.w),SMPS_RAM.v_variables.v_speeduptempo(a6)
	bclr	#0,d1				; Clownacy | Is this a forced-PAL tempo song? (we clear PAL tempo flag so it doesn't interfere later on)
	beq.s	.nopalmode
	bset	#f_force_pal_tempo,SMPS_RAM.v_variables.misc_flags(a6) ; Clownacy | If so, set flag
...
.nopalmode:
	...
	move.b	d0,SMPS_RAM.v_variables.v_tempo_mod(a6)
	tst.b	SMPS_RAM.v_variables.f_speedup(a6)
	beq.s	.nospeedshoes
	move.b	SMPS_RAM.v_variables.v_speeduptempo(a6),d0
; loc_72068:
.nospeedshoes:
	move.b	d0,SMPS_RAM.v_variables.v_main_tempo(a6)
	moveq	#0,d1
	move.b	d1,SMPS_RAM.v_variables.v_main_tempo_timeout(a6)	; Clownacy | Cleared to avoid unintended overflow on first frame of playback
	move.b	#5,SMPS_RAM.v_variables.v_pal_audio_countdown(a6)	; Clownacy | "reset PAL update tick to 5 (update immediately)"
	movea.l	a4,a3
...
; Sound_A0toCF:
Sound_PlaySFX:
	tst.b	SMPS_RAM.v_variables.f_1up_playing(a6)		; Is 1-up playing?
	bne.w	.clear_sndprio			; Exit is it is
;	tst.b	SMPS_RAM.v_variables.v_fadeout_counter(a6)		; Is music being faded out?	; Clownacy | S2's driver doesn't bother with this
;	bne.w	.clear_sndprio			; Exit if it is
	btst	#f_fadein_flag,SMPS_RAM.v_variables.misc_flags(a6)	; Is music being faded in?
	bne.w	.clear_sndprio			; Exit if it is
	bclr	#f_spindash_lastsound,SMPS_RAM.v_variables.misc_flags(a6)
	cmpi.b	#SndID_Ring,d7			; Is ring sound	effect played?
	bne.s	.sfx_notRing			; If not, branch
	btst	#v_ring_speaker,SMPS_RAM.v_variables.misc_flags(a6)	; Is the ring sound playing on right speaker?
	bne.s	.gotringspeaker			; Branch if not
	move.b	#SndID_RingLeft,d7		; Play ring sound in left speaker
; loc_721EE:
.gotringspeaker:
	bchg	#v_ring_speaker,SMPS_RAM.v_variables.misc_flags(a6)	; Change speaker
; Sound_notB5:
.sfx_notRing:
	...
	btst	#f_push_playing,SMPS_RAM.v_variables.misc_flags2(a6)	; Is pushing sound already playing?
	bne.w	.locret				; Return if not
	bset	#f_push_playing,SMPS_RAM.v_variables.misc_flags2(a6)	; Mark it as playing
    endif
; Sound_notA7:
.sfx_notPush:
	...
	bchg	#v_gloop_toggle,SMPS_RAM.v_variables.misc_flags(a6)	; Z80 cpl
...
.sfx_notgloop:
	...
	move.b	SMPS_RAM.v_variables.v_spindash_pitch(a6),d0		; Store extra frequency
	tst.b	SMPS_RAM.v_variables.v_spindash_timer(a6)		; Is the Spin Dash timer active?
...
.sfx_timeractive:
	...
	move.b	d0,SMPS_RAM.v_variables.v_spindash_pitch(a6)		; Otherwise, set new frequency

.sfx_limitreached:
	bset	#f_spindash_lastsound,SMPS_RAM.v_variables.misc_flags(a6)	; Set flag
	move.b	#60,SMPS_RAM.v_variables.v_spindash_timer(a6)		; Set timer
...
.sfx_notspindashrev:
	...
	moveq	#0,d0
	move.b	SMPS_RAM.v_variables.v_current_contsfx(a6),d0
	cmp.b	d7,d0					; Is this the same continuous sound that was playing?
	bne.s	.sfx_notsame				; If not, branch
	bset	#f_continuous_sfx,SMPS_RAM.v_variables.misc_flags(a6)	; Set flag for continuous playback mode
	subi.b	#SndID__First,d7
	add.w	d7,d7				; Convert sfx ID into index
	add.w	d7,d7
	lea	(SoundIndex).l,a0
	movea.l	(a0,d7.w),a0
	move.b	3+2(a0),SMPS_RAM.v_variables.v_contsfx_channels(a6)	; Save number of channels in SFX
	rts

.sfx_notsame:
	bclr	#f_continuous_sfx,SMPS_RAM.v_variables.misc_flags(a6)	; Clear flag for continuous playback mode
	move.b	d7,SMPS_RAM.v_variables.v_current_contsfx(a6)		; Mark this as the current continuous SFX
...
.clearsfxtrackram:
    ...
    if SMPS_EnableSpinDashSFX
	btst	#f_spindash_lastsound,SMPS_RAM.v_variables.misc_flags(a6)	; Is the Spin Dash sound playing?
	beq.s	.notspindash				; If not, branch
	move.b	SMPS_RAM.v_variables.v_spindash_pitch(a6),d0
...
; loc_722C6:
.clear_sndprio:
	clr.b	SMPS_RAM.v_variables.v_sndprio(a6)	; Clear priority
	rts
; ===========================================================================
; ---------------------------------------------------------------------------
...
;	tst.b	SMPS_RAM.v_variables.v_fadeout_counter(a6)		; Is music being faded out?	; Clownacy | S2's driver didn't bother with this in Sound_PlaySFX
...
;	bsr.s	StopSpecSFX
;    endif
	move.b	#3,SMPS_RAM.v_variables.v_fadeout_delay(a6)		; Set fadeout delay to 3
	move.b	#$28,SMPS_RAM.v_variables.v_fadeout_counter(a6)	; Set fadeout counter
	clr.b	SMPS_RAM.v_variables.f_speedup(a6)			; Disable speed shoes tempo
	rts
...
; sub_72504:
DoFadeOut:
	move.b	SMPS_RAM.v_variables.v_fadeout_delay(a6),d0	; Has fadeout delay expired?
	beq.s	.continuefade		; Branch if yes
	subq.b	#1,SMPS_RAM.v_variables.v_fadeout_delay(a6)
	rts
; ===========================================================================
; loc_72510:
.continuefade:
	subq.b	#1,SMPS_RAM.v_variables.v_fadeout_counter(a6)	; Update fade counter
	beq.w	StopSoundAndMusic		; Branch if fade is done
	move.b	#3,SMPS_RAM.v_variables.v_fadeout_delay(a6)		; Reset fade delay
	lea	SMPS_RAM.v_music_dac_track(a6),a5
	tst.b	SMPS_Track.PlaybackControl(a5)	; Is track playing?
...
StopSoundAndMusic:
	...
	bsr.w	WriteFMI
	movea.l	a6,a0
	move.w	#((SMPS_RAM.v_spcsfx_track_ram_end-SMPS_RAM.v_variables)/4)-1,d0	; Clear all variables and track data (don't really care about clearing the music track backup)
.skipclearsfx:
	moveq	#0,d2
; loc_725B6:
.clearramloop:
...
InitMusicPlayback:
	movea.l	a6,a0
	; Save several values
	move.b	SMPS_RAM.v_variables.v_sndprio(a6),d1
	move.b	SMPS_RAM.v_variables.f_1up_playing(a6),d2
	move.b	SMPS_RAM.v_variables.f_speedup(a6),d3
	move.b	SMPS_RAM.v_variables.v_fadein_counter(a6),d4
	move.l	SMPS_RAM.v_variables.v_playsnd1(a6),d5
	move.w	#((SMPS_RAM.v_music_track_ram_end-SMPS_RAM.v_variables)/4)-1,d0	; Clear $220 bytes: all variables and music track data
	moveq	#0,d6
...
.clearramloop:
	...
	; Clownacy | Make sure the last few bytes get cleared
    if (SMPS_RAM.v_music_track_ram_end-SMPS_RAM.v_variables)&2
	move.w	d6,(a0)+
    endif
    if (SMPS_RAM.v_music_track_ram_end-SMPS_RAM.v_variables)&1
	move.b	d6,(a0)+
    endif

	; Restore the values saved above
	move.b	d1,SMPS_RAM.v_variables.v_sndprio(a6)
	move.b	d2,SMPS_RAM.v_variables.f_1up_playing(a6)
	move.b	d3,SMPS_RAM.v_variables.f_speedup(a6)
	move.b	d4,SMPS_RAM.v_variables.v_fadein_counter(a6)
	move.l	d5,SMPS_RAM.v_variables.v_playsnd1(a6)
	move.b	d6,SMPS_RAM.v_variables.v_playsnd0(a6)	; set music to $00 (silence)
...
TempoWait:	; Clownacy | Ported straight from S3K's Z80 driver
	move.b	SMPS_RAM.v_variables.v_main_tempo(a6),d0				; Get current tempo value
	add.b	d0,SMPS_RAM.v_variables.v_main_tempo_timeout(a6)
...
; Sound_E2:
SpeedUpMusic:
	tst.b	SMPS_RAM.v_variables.f_1up_playing(a6)
	bne.s	SpeedUpMusic_1up
	move.b	SMPS_RAM.v_variables.v_speeduptempo(a6),SMPS_RAM.v_variables.v_main_tempo(a6)
	move.b	#$80,SMPS_RAM.v_variables.f_speedup(a6)
	rts
; ===========================================================================
; loc_7263E: .speedup_1up:
SpeedUpMusic_1up:
	move.b	SMPS_RAM.v_1up_variables.v_speeduptempo(a6),SMPS_RAM.v_1up_variables.v_main_tempo(a6)
	move.b	#$80,SMPS_RAM.v_1up_variables.f_speedup(a6)
	rts
; ===========================================================================
; ---------------------------------------------------------------------------
...
SlowDownMusic:
	...
	move.b	SMPS_RAM.v_variables.v_tempo_mod(a6),SMPS_RAM.v_variables.v_main_tempo(a6)
	clr.b	SMPS_RAM.v_variables.f_speedup(a6)
	rts
; ===========================================================================
; loc_7266A: .slowdown_1up:
SlowDownMusic_1up:
	move.b	SMPS_RAM.v_1up_variables.v_tempo_mod(a6),SMPS_RAM.v_1up_variables.v_main_tempo(a6)
	clr.b	SMPS_RAM.v_1up_variables.f_speedup(a6)
	rts
...
; sub_7267C:
DoFadeIn:
	tst.b	SMPS_RAM.v_variables.v_fadein_delay(a6)	; Has fadein delay expired?
	beq.s	.continuefade		; Branch if yes
	subq.b	#1,SMPS_RAM.v_variables.v_fadein_delay(a6)
	rts
; ===========================================================================
; loc_72688:
.continuefade:
	tst.b	SMPS_RAM.v_variables.v_fadein_counter(a6)	; Is fade done?
	beq.s	.fadedone		; Branch if yes
	subq.b	#1,SMPS_RAM.v_variables.v_fadein_counter(a6)	; Update fade counter
	move.b	#2,SMPS_RAM.v_variables.v_fadein_delay(a6)	; Reset fade delay
	lea	SMPS_RAM.v_music_dac_track(a6),a5
	tst.b	SMPS_Track.PlaybackControl(a5) ; Is track playing?
	bpl.s	.fadefm			; Branch if not
...
.fadedone:	; Modified version of MJ's original DAC fade-in fix
	bclr	#f_fadein_flag,SMPS_RAM.v_variables.misc_flags(a6)		; Stop fadein
...
; loc_72AF2:
cfSetCommunication:
	move.b	(a4)+,SMPS_RAM.v_variables.v_communication_byte(a6)	; Set otherwise unused communication byte to parameter
	rts
; ===========================================================================
; loc_72AF8:
cfJumpReturn:
...
cfFadeInToPrevious:
	...
	btst	#f_updating_dac,SMPS_RAM.v_variables.misc_flags(a6)	; Clownacy | Is this running on the DAC channel
...
.restoreplaybackloop:
	...
	movea.l	a5,a3
	moveq	#$28,d6
	sub.b	SMPS_RAM.v_variables.v_fadein_counter(a6),d6			; If fade already in progress, this adjusts track volume accordingly
	lea	SMPS_RAM.v_music_dac_track(a6),a5
...
.nextpsg:
	...
	bset	#f_fadein_flag,SMPS_RAM.v_variables.misc_flags(a6)	; Trigger fade-in
	move.b	#$28,SMPS_RAM.v_variables.v_fadein_counter(a6)	; Fade-in delay
	clr.b	SMPS_RAM.v_variables.f_1up_playing(a6)
...
cfChangeFMVolume:
	...
	btst	#f_updating_dac,SMPS_RAM.v_variables.misc_flags(a6)
	bne.w	SetDACVolume
	bra.w	SendVoiceTL
; ===========================================================================
; loc_72BAE:
...
; loc_72BC6:
cfSetTempo:
	move.b	(a4),SMPS_RAM.v_variables.v_main_tempo(a6)		; Set main tempo
	move.b	(a4)+,SMPS_RAM.v_variables.v_main_tempo_timeout(a6)	; And reset timeout (!)
	rts
; ===========================================================================
; loc_72BD0:
cfSetTempoMod:
...
; loc_72BEE:
cfClearPush:
	bclr	#f_push_playing,SMPS_RAM.v_variables.misc_flags2(a6)	; Allow push sound to be played once more
	rts
    endif
; ===========================================================================
    if SMPS_EnableSpecSFX
...
cfStopTrack:
	...
	btst	#f_updating_dac,SMPS_RAM.v_variables.misc_flags(a6)	; Is this the DAC we are updating?
...
; loc_72D78:
.stoppedchannel:
	tst.b	SMPS_RAM.v_variables.f_voice_selector(a6)		; Are we updating SFX?
...
; cfPlaySoundByIndex
cfPlaySound:
	move.b	(a4)+,SMPS_RAM.v_variables.v_playsnd2(a6)
	rts
; ===========================================================================
...
;
    if SMPS_EnableContSFX
cfLoopContinuousSFX:
	btst	#f_continuous_sfx,SMPS_RAM.v_variables.misc_flags(a6)	; Is the flag for continuous playback mode set?
	bne.s	.continuousmode				; If so, branch
	clr.b	SMPS_RAM.v_variables.v_current_contsfx(a6)			; Communicate that there is no continuous SFX playing
	addq.w	#2,a4					; Clownacy | Advance reading counter to skip the address
	rts

.continuousmode:
	subq.b	#1,SMPS_RAM.v_variables.v_contsfx_channels(a6)		; Mark one channel as processed
	bne.w	cfJumpTo				; If that wan't the last channel, branch
	bclr	#f_continuous_sfx,SMPS_RAM.v_variables.misc_flags(a6)	; If it was, clear flag for continuous playback mode...
